...
with p.name as nev, count(m.title) as db //segéd lekérdezés, mindig új név kell
where db > 1 //having
return nev, db
```
...
//alt1: match (p:Person)-[:ACTED_IN]->(m:Movie)
// match (p:Person)-[:DIRECTED]->(m:Movie) (ugyanazok a változók)

//alt2: match (p:Person)-[:ACTED_IN]->(m:Movie)
//where exists ((p:Person)-[:DIRECTED]->(m:Movie)) - lehet tagadni is
```
...
for (p:Person)
on (p.name, p.born) //név, azon belül szül.év alapján sorbarendezés
```
