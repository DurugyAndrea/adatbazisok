...
select title
from movies_and_tv
where type = 'Movie' AND release_year = 2018 //where-ben vagy kulcs vagy indexelt oszlop
limit 3;
```
...
```js
select title from movies_and_tv where country = 'United States' limit 10 allow filtering; //itt működik az allow filtering

select type, release_year, count(*) from movies_and_tv group by type, release_year limit 10; //group by-nál kell a partíció kulcs (type és release_year) - sorrend sem mindegy!
```
...
insert into Szemely (nev, szulev, foglalkozas) values ('Nagy Ivó', 1999, 'diak');
...
where foglalkozas = 'lakatos' or foglalkozas = 'pincer'; //OR nincs: where foglalkozas in ('pincer', 'lakatos');
